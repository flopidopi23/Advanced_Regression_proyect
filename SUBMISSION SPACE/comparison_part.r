...
# Fit a smoothing spline to squared residuals to estimate local variance
var_model <- gam(svm_residuals^2 ~ s(x), data = data)

# Predict local variance for test data
pred_var <- predict(var_model, testing_data)
pred_var[pred_var < 0] <- 0  # Ensure non-negative

# Calculate intervals using constant coverage
z_value <- qnorm(0.975)  # 95% interval
svm_lower <- svm_pred - z_value * sqrt(pred_var)
svm_upper <- svm_pred + z_value * sqrt(pred_var)
