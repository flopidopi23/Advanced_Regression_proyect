...
png("svm_tuning_performance.png", width = 10, height = 7, units = "in", res = 300) 
par(mar = c(5, 5, 4, 2)) 
plot(OptModelsvm) 
dev.off() 
...
# Save the plot with the test results
p6_svm
ggsave("plot_residuals_hist_svr.png", p6_svm, width = 10, height = 6, dpi = 300) #JC DID THIS
...
# USE BOOTSTRAP HERE!!!!
# Improved SVM prediction intervals using bootstrap
# Set bootstrap parameters
n_bootstrap <- 1000
set.seed(666420)  # Maintain consistency with previous analyses

# Create matrix to store bootstrap predictions
bootstrap_preds <- matrix(0, nrow = nrow(testing_data), ncol = n_bootstrap)
...
for (i in 1:n_bootstrap) {
    ...
    # Train SVM on bootstrap sample using the same parameter settings as the best model
    boot_model <- svm(y ~ x, 
                      data = boot_data,
                      epsilon = OptModelsvm$best.parameters$epsilon,
                      cost = OptModelsvm$best.parameters$cost)
    
    # Predict on test data
    bootstrap_preds[, i] <- predict(boot_model, testing_data)
}

# Calculate mean prediction for each test point
svm_pred_mean <- rowMeans(bootstrap_preds)

# Calculate prediction intervals 
# We need to account for both model uncertainty and inherent noise
# First, estimate residual variance from original model
sigma_hat <- sd(BstModel$residuals)

# For each test point, get the standard deviation of bootstrap predictions
# This captures model uncertainty
model_uncertainty <- apply(bootstrap_preds, 1, sd)

# Combine both sources of uncertainty
total_uncertainty <- sqrt(model_uncertainty^2 + sigma_hat^2)

# Calculate 95% prediction intervals
svm_lower <- svm_pred - 1.96 * total_uncertainty
svm_upper <- svm_pred + 1.96 * total_uncertainty
...
summary_df <- data.frame(
    ...
    Avg_Interval_Width = c(mean(pspline_upper - pspline_lower), mean(svm_predictions$upper - svm_predictions$lower))
)
